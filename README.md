...
- `max_buffer_size_in_mb` (int): The maximum memory buffer size in MB. Default value is `1024`. This is the maximum size of the chunks that will be stored in memory on downloads. If this value is to low compared off the `max_download_workers` you won't be able to achieve good speeds since there will not be space to save chunks at the cache.
